...
export const SubcategoryMenu = ({ category, isOpen, position }: SubcategoryMenuProps) => {
    ...
    let textColor = category.textColor || '#333'
                ...
                <div>
                    {category.subcategories?.map((subcategory: Category) => (
                        <Link
                            ...
                            href='/'
                            className={cn("w-full text-left p-4 hover:bg-black/50 hover:text-white flex justify-between items-center underline font-medium")}
                        >
                            {subcategory.name}
                        </Link>
                    ))}
